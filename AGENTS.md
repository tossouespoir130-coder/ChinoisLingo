...
- **Règle Permanente de Distinction des 3 Rôles et Nommages d'Espoir** :
  ...
  - **Rôle 2 — Formateur** (Rubrique Formations, cours vidéos, podcasts animés) : **`Espoir Chinois`** (*« Formateur : Espoir Chinois »*).
  - **Rôle 3 — Personnage dans les Dialogues, Histoires et Vidéos de cours** :
...
4. **Intégralité, Synchronisation Audio et Découpage Rythmique des Paroles (Lyrics)** :
   ...
   - **Découpage Fin à Chaque Pause / Virgule (Saut de Ligne Obligatoire)** :
    ...
    - **Règle Permanente de Miniature YouTube Directe pour TOUTES les Chansons** : Sur TOUTES les cartes de chansons du catalogue et dans les prévisualisations, l’image d’illustration DOIT utiliser directement la miniature officielle de YouTube (`https://img.youtube.com/vi/<youtubeId>/hqdefault.jpg`), garantissant une cohérence visuelle parfaite avec les Masterclass et vidéos.
    - **Règle Permanente d'Épuration des Lyrics & Suppression du Nom de l'Artiste / Speaker dans les Paroles** : Pour TOUTES les chansons de la plateforme, ne jamais afficher ni inclure de nom d’artiste ou de champ `speaker` devant les vers dans les couplets ou refrains. Conserver exclusivement les balises de sections musicales (`🎵 Refrain`, `🎵 Pré-refrain`, `Couplet 1`, `Pont`, `Outro`).
    - **Règle Permanente de Synthèse Vocale ElevenLabs Obligatoire pour TOUS les Lyrics de Chansons** : Pour TOUTE chanson ajoutée ou mise à jour, générer systématiquement les pistes audio ElevenLabs pour les paroles parlées (clips individuels par vers `/audio/readings/${songId}_${sentenceId}.mp3`, master unifié `/audio/readings/${songId}.mp3` et fichier de métadonnées `/audio/readings/${songId}_meta.json`). L’audio doit être léger, clair, doux et agréable à écouter, permettant à l’apprenant de cliquer sur n’importe quel vers pour entendre la prononciation exacte du mandarin.
